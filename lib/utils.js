function writeHttp(path, gitfile) {
    const url = "https://raw.githubusercontent.com/tairasoul/jsMacros-projects/main/lib/" + gitfile;
...
function checkMelonRindFork(path) {
    if (!FS.exists("aMelonRind/" + path)) {
        const url = "https://raw.githubusercontent.com/tairasoul/JsMacros-scripts/main/src/lib/" + path;
        const http = Request.get(url);
        const text = http.text();
